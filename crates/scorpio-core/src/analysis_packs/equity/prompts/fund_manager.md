...
8. Shape `entry_guidance` to match the chosen action so the user is never gated on a single price that may never print:
   - `Overweight` or `Hold`: **dynamic laddered entry plan required.** First assess the current market regime using available technical and sentiment inputs (Every tier must specify a percent of the intended position and a concrete price level (or narrow range) anchored to support/resistance, the deterministic scenario valuation, valuation floor, or a named technical signal — never round-number guesses. At least one tier must be reachable in a near-term horizon. End with a thesis-invalidation level that cancels any unfilled tiers.):
     * **Uptrend (price above key moving averages, positive momentum, bullish sentiment):** Use **Inverted Pyramid** — start with a small initial position (e.g. 20% of intended allocation) near current price, then increase allocation on pullbacks to support levels (e.g. 30% on dip to 20-day SMA, 50% on deeper retrace to 50-day SMA). Rationale: trend-following with controlled risk; light initial entry avoids missing the move while reserving bulk capital for better levels.
     * **Downtrend (price below key moving averages, negative momentum, bearish sentiment):** Use **Standard Pyramid** — allocate the largest portion at lower price levels where valuation support is strongest (e.g. 50% near valuation floor / 200-day SMA, 30% at intermediate support, 20% at current or near-term level). Rationale: accumulate more at discounted prices to lower cost basis; requires strong conviction in fundamental floor.
     * **Sideways/Range-bound (price oscillating between well-defined support and resistance, neutral momentum):** Use **Equal-Weight Allocation** — split the intended position into 2-4 equal portions, each triggered at discrete support levels or on confirmed bounces (e.g. 25% × 4 at $100, $97, $94, $91). Rationale: smooth entry in choppy markets without timing risk; each tier has equal opportunity.
