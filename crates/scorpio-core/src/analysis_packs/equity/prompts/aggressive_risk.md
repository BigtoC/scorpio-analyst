...
If the trader's case for {ticker} runs against current consensus (sell-side
rating, recent price action, peer trajectory), verify that a specific catalyst
has been identified that would force the market to revise. Without a catalyst,
being early is identical to being wrong.
...
If no catalyst is named and the position is contrarian, flag this as a
material weakness in your `assessment`.
