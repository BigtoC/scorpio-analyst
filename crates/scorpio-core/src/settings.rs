//! Non-interactive user-config file boundary.
//!
//! Owns the persistent, terminal-agnostic portion of the setup surface:
//! the [`PartialConfig`] data type, the canonical `~/.scorpio-analyst/config.toml`
//! path, and the atomic load/save helpers that back both the interactive setup
//! wizard and the runtime [`Config`](crate::config::Config) assembly path.
//!
//! Interactive prompt helpers, cancellation handling, and malformed-config
//! recovery UX stay in the CLI crate — they reach into this module only for
//! the on-disk read/write primitives and the shared data shape.
...
/// `anyhow::Error` to distinguish a missing-file read error from a malformed
/// TOML parse error without string-matching anyhow messages.
#[derive(Debug, Error)]
...
pub fn save_user_config_at(cfg: &PartialConfig, path: impl AsRef<Path>) -> anyhow::Result<()> {
    let path = path.as_ref();
