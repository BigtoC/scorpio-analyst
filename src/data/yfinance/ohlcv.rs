//! Yahoo Finance OHLCV client and VIX volatility fetcher.
//!
...
//!
//! Also exposes [`fetch_vix_data`], which fetches 60 days of `^VIX` daily
//! candles and computes a small set of deterministic metrics — no LLM
//! involvement.  The result is stored on [`TradingState::market_volatility`]
//! before the analyst fan-out so every downstream agent sees the same
//! volatility context.
