...
- [ ] 2.1 Add the six additive fields to `src/state/trading_state.rs`:
...
- [ ] 2.2 Initialize all six fields to `None` in `TradingState::new`.
- [ ] 2.3 Run `cargo build` and update every `TradingState { ... }` literal that becomes incomplete.
...
- [ ] 2.4 Add a context-bridge round-trip test proving the new evidence/coverage/provenance fields survive
  `serialize_state_to_context` / `deserialize_state_from_context`.
- [ ] 2.5 Add a snapshot round-trip test proving the new evidence/coverage/provenance fields survive save/load.
- [ ] 2.6 Run:

  - `cargo test --lib workflow::context_bridge -- --nocapture`
  - `cargo test --lib workflow::snapshot -- --nocapture`

- [ ] 2.7 Commit: `feat: extend TradingState with evidence and reporting fields`.

## 3. AnalystSyncTask Dual-Write And Derivation Rules

- [ ] 3.1 Add Stage 1 source-mapping helpers or constants in `src/workflow/tasks/analyst.rs`:
...
- [ ] 3.2 Update `AnalystSyncTask` so each successful merged analyst result dual-writes both the legacy field and the
  corresponding `evidence_*` field with `quality_flags: vec![]`.
- [ ] 3.3 Derive `DataCoverageReport` from the typed `evidence_*` fields only, using the fixed required input order
...
- [ ] 3.4 Derive `ProvenanceSummary` from the providers attached to evidence records that are actually present on the
  continue path.
...
- [ ] 3.5 Extend `analyst_sync_all_succeed_returns_continue` in `src/workflow/tasks/tests.rs` to assert:
...
- [ ] 3.6 Add a new regression test exercising the one-failure continue path, for example
...
- [ ] 3.7 Run `cargo test --lib workflow::tasks -- --nocapture`.
- [ ] 3.8 Commit: `feat: dual-write analyst evidence and coverage metadata in AnalystSyncTask`.

## 4. Shared Prompt Builders And Downstream Consumer Boundaries

- [ ] 4.1 Add `build_evidence_context(state: &TradingState) -> String` to `src/agents/shared/prompt.rs`.
...
- [ ] 4.2 Add `build_data_quality_context(state: &TradingState) -> String` to `src/agents/shared/prompt.rs`.
...
- [ ] 4.3 Add unit tests in `src/agents/shared/prompt.rs` proving both new builders contain expected keys when state is
  populated and return non-empty fallback text when the relevant fields are absent.

- [ ] 4.4 In `src/agents/researcher/common.rs`, extend the existing `build_analyst_context(state)` helper so it appends
...
- [ ] 4.5 In `src/agents/risk/common.rs`, extend the existing `build_analyst_context(state)` helper the same way. This
  is the preferred single injection point for the risk persona agents and the risk moderator.

- [ ] 4.6 In `src/agents/trader/mod.rs::build_prompt_context`, append the new typed evidence and data-quality blocks at
  the existing dynamic prompt-construction site. Keep the legacy analyst report fields during the Stage 1 dual-write
  period.

- [ ] 4.7 In `src/agents/fund_manager/prompt.rs::build_prompt_context` / `build_user_prompt`, append the new typed
...
- [ ] 4.8 Add string-contains tests covering the actual injection boundaries:
...
- [ ] 4.10 Record cross-owner approval and owner awareness.
...
- [ ] 4.11 Commit: `feat: inject typed evidence and quality context into downstream prompts`.

## 5. Verification

- [ ] 5.1 Run `cargo fmt -- --check`.
- [ ] 5.2 Run `cargo clippy --all-targets -- -D warnings`.
- [ ] 5.3 Run `cargo nextest run --all-features --locked`.
- [ ] 5.4 Run `cargo build`.
