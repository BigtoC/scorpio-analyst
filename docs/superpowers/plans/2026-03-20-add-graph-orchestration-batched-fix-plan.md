...
- [ ] Add a failing test that forces a known node failure and asserts `TradingError::GraphFlow` contains the real task id and real phase name instead of generic labels.
...
- [ ] Refactor workflow error mapping in `src/workflow/pipeline.rs` so runner/task failures preserve real node identity when known.
- [ ] Ensure workflow-surfaced graph errors do not collapse into generic `pipeline_execution`, `flow_runner`, or `task_failure` placeholders.
...
- [ ] Add failing tests for `max_debate_rounds = 0` and `max_risk_rounds = 0` that assert:
  - counters stay at `0`
...
- [ ] Update `DebateModeratorTask` so zero-round routing does not increment `KEY_DEBATE_ROUND` or materialize a fake round entry.
- [ ] Update `RiskModeratorTask` so zero-round routing does not increment `KEY_RISK_ROUND` or materialize a fake round entry.
...
- [ ] Add failing tests for analyst child tasks that prove context/state deserialization or orchestration write failures surface as workflow errors rather than being silently downgraded into analyst degradation.
- [ ] Run the targeted tests and confirm they fail.
- [ ] Use exact test names when adding them, for example one deserialization-failure case and one context-write-failure case, and run those exact tests while iterating.
- [ ] Refactor analyst child wrappers so true orchestration corruption fails closed while normal upstream analyst failures still follow the degradation policy.
- [ ] Keep the intended degradation behavior for real analyst runtime failures: `1` failure continues, `2+` failures abort.
- [ ] Re-run the targeted tests and confirm they pass.
...
- [ ] Record any follow-up findings before moving to Batch 2.
...
- [ ] Add a workflow-local deterministic test seam inside `src/workflow/` rather than changing agent-file ownership. Prefer dependency injection or test-only constructors in the workflow layer over cross-owner edits.
- [ ] Add a failing integration test that successfully executes `TradingPipeline::run_analysis_cycle()` and asserts:
...
- [ ] Run the targeted integration test and confirm it fails.
...
- [ ] Implement the smallest workflow-local seam needed to make the success-path test deterministic without live provider calls.
...
- [ ] Add explicit assertions for real graph routing on `max_debate_rounds = 0/1/N` and `max_risk_rounds = 0/1/N` using the workflow-local seam from Task 5.
- [ ] Add explicit assertions for phase-usage ordering across analyst, researcher round(s), moderation, trader, risk round(s), moderation, and fund manager.
- [ ] Re-run the targeted tests and confirm they pass.
...
- [ ] Add explicit assertions that two invocations with the same caller input state produce distinct saved execution IDs.
- [ ] Add explicit assertions that snapshots for phases 1-5 are loadable and contain boundary-appropriate state.
- [ ] Re-run the workflow integration tests.
...
- [ ] Stop here for review before touching security-hardening files in Batch 3.
...
- [ ] Add failing tests that prove raw provider/model text is not surfaced through workflow-level `TradingError::GraphFlow` messages.
- [ ] Implement a workflow-local sanitizer with the same behavior goals as the provider layer unless the approved cross-owner list is expanded.
- [ ] Re-run the targeted tests and confirm they pass.
...
- [ ] Do not edit `src/agents/researcher/mod.rs` or any fund-manager file in this batch unless the OpenSpec proposal/spec and cross-owner approvals are expanded first.
- [ ] Record agent-level persistence redaction as deferred follow-up if workflow-local sanitization and snapshot hardening do not fully close the security concern.
...
- [ ] Add a failing test for an unexpected runaway workflow condition or corrupted round/step state.
- [ ] Add a hard workflow step ceiling so orchestration fails closed instead of looping indefinitely on bad session state.
- [ ] Add failing tests for hostile or malformed snapshot path edge cases; the snapshot path is required to remain configurable by spec.
- [ ] Tighten snapshot path handling enough to reject clearly unsafe or malformed cases while keeping supported paths working.
- [ ] Re-run targeted tests and confirm they pass.

### Task 12: Verify Batch 3

- [ ] Run focused workflow and persistence tests.
...
- [ ] Batch 1 closes the main no-go blockers inside `src/workflow/`
- [ ] Batch 2 proves successful `run_analysis_cycle()` execution and real routing/accounting coverage within the currently approved file boundary
- [ ] Batch 3 closes sanitization / persistence / runaway-execution concerns
