...
Replace the Stage 1 enrichment placeholder path with real normalized transcript, consensus-estimates, and event-news payloads behind the provider-agnostic adapter contracts.
...
- R1. Implement concrete providers behind the transcript, estimates, and event-news adapter contracts.
...
- `src/data/finnhub.rs` already provides the strongest existing base for earnings/event-oriented integration work.
...
**Goal:** Turn contract-only adapters into concrete transcript/estimates/event providers.
...
**Goal:** Make the new enrichment payloads visible to prompt, evidence, and report consumers.
...
- **Interaction graph:** provider-capability-enabled startup -> enrichment fetch/normalize -> typed cache payloads -> prompt/report/evidence consumption.
