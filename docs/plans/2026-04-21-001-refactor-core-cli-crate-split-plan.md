...
- `scorpio-analyst` — existing crate name retained, now a CLI-only binary crate (clap/inquire command surface, setup wizard, update notices, terminal banner, and terminal report formatting). Depends on `scorpio-core`.

The split is strictly behavior-preserving: no user-visible changes to CLI commands, binary name, config semantics, or pipeline behavior. The refactor lands through compile-green stages so each unit can be reviewed and CI-validated independently.
...
- R12 — No opportunistic cleanup, renames, or broad API redesign.
...
- **Binary artifact naming** — The release pipeline (`.github/workflows/release.yml:101,107,114,119`) builds `target/${{ matrix.target }}/release/scorpio-analyst`, strips it, copies to `release-stage/scorpio`, and archives as `scorpio-<triple>.tar.gz`. Install scripts (`install.sh`, `install.ps1`) expect that archive name and place the binary at `~/.local/bin/scorpio`. The CLI package must keep the name `scorpio-analyst` so the build output path is unchanged.
- **Test imports across `tests/`** — Every integration test imports `use scorpio_analyst::{state, error, workflow, providers, data, ...}`. Once those modules move to core (Unit 4), imports switch to `scorpio_core::...`. Affected files: `tests/foundation_edge_cases.rs`, `tests/state_roundtrip.rs`, `tests/workflow_pipeline_e2e.rs`, `tests/workflow_pipeline_structure.rs`, `tests/workflow_observability_tasks.rs`, `tests/workflow_pipeline_accounting.rs`, `tests/workflow_observability_pipeline.rs`, and all files in `tests/support/`.
...
- **Binary name preservation strategy.** Keep CLI crate package name `scorpio-analyst` → build output at `target/<triple>/release/scorpio-analyst` → release pipeline and install scripts untouched. Resolves R8.
...
```mermaid
flowchart LR
    MAIN[main.rs<br/>bin: scorpio-analyst] --> CLI[cli::*]
    CLI --> REPORT[report::*]
...
**Approach:**
- Preserve the current package name `scorpio-analyst` and version so the build output path at `target/<triple>/release/scorpio-analyst` is unchanged. Release pipeline and install scripts are untouched in this unit.
...
- [ ] **Unit 2: Introduce empty `scorpio-core` crate and wire it as a dependency of `scorpio-analyst`**

**Goal:** Add a second workspace member `crates/scorpio-core` with an empty library surface, and declare `scorpio-core = { path = "../scorpio-core" }` in `scorpio-analyst`'s Cargo.toml so Units 3–5 can move modules into it one slice at a time.
...
**Verification:**
- `cargo tree -p scorpio-analyst | grep scorpio-core` shows the path dependency is wired.
...
- Integration — `cargo run -p scorpio-analyst -- setup` + `cargo run -p scorpio-analyst -- analyze AAPL` behave identically to before (same prompts, same report format, same exit codes for invalid symbols / missing config).
- Integration — `scorpio upgrade` (via `cargo run -p scorpio-analyst -- upgrade` in a mock mode) still executes the full self-update path without relying on CLI-only globals.
...
- Edge case — `use scorpio_analyst::state::TradingState` in any remaining CLI test compiles (via re-export) OR has been rewritten to `scorpio_core::state::TradingState`. Grep `scorpio_analyst::(state|workflow|providers|data|error|config|rate_limit|agents|indicators|analysis_packs|observability|backtest)` in tests to verify the migration.
...
- `cargo clippy --workspace --all-targets -- -D warnings` passes.
- A manual `cargo run -p scorpio-analyst -- analyze AAPL` smoke with minimal config and `SCORPIO__LLM__MAX_DEBATE_ROUNDS=1` still produces the existing banner, proceeds through the pipeline, and prints the same report shape.
...
- **Unchanged invariants:**
  - Binary artifact path: `target/<triple>/release/scorpio-analyst` → staged as `scorpio` → archived as `scorpio-<triple>.{tar.gz,zip}`. Install scripts + release workflow untouched.
...
| `scorpio upgrade` self-update flow regresses because `CARGO_PKG_VERSION` now reads a different package                                                       | Low        | Medium | CLI crate keeps the name `scorpio-analyst`; `env!("CARGO_PKG_VERSION")` in `main.rs` resolves to the CLI crate's version. Verify with a smoke `scorpio --version` in Unit 5.                                                        |
| Test imports stay `use scorpio_analyst::...` and silently pass due to re-exports, masking cross-crate discipline                                             | Medium     | Low    | No `pub use` bulk re-exports at the core root (R24). Tests that reference core types must explicitly import from `scorpio_core::...` — the compiler enforces.                                                                       |
