...
## Core Boundary Guidance
| Concern                                                                         | Recommended placement     | Why                                                                                                               |
|---------------------------------------------------------------------------------|---------------------------|-------------------------------------------------------------------------------------------------------------------|
| Runtime settings contract and effective runtime assembly                        | Stable core boundary      | Shared behavior today already crosses into CLI-owned code; future surfaces should reuse the same contract         |
| Analysis execution entrypoint                                                   | Stable core facade        | Future surfaces should call one application service instead of rebuilding provider/client/pipeline setup          |
| Agents, workflow tasks, provider factories, raw data clients, rig tool wrappers | Core internals by default | These are likely to churn and should not become the main shared API unless a real second consumer proves the need |
| Interactive setup prompts, banners, update notices, terminal report rendering   | CLI                       | Delivery-specific behavior that should not shape the stable core API                                              |
...
- R12. The first slice moves only what is required to enforce crate ownership and dependency direction; opportunistic cleanup, broad API redesign, and non-essential renames stay out of scope.
...
**Core API Shape**
- R16. The stable shared API of the core crate is application-centered: callers depend on a thin facade for runtime preparation and analysis execution rather than constructing providers, clients, and pipeline orchestration themselves.
- R17. In the first slice, that thin facade stays minimal and close to the existing analysis/config assembly path; broader API curation is deferred until a real second consumer exists.
- R18. Runtime settings loading, persisted user-settings file behavior, and effective runtime assembly live behind a core-owned settings boundary rather than in CLI-owned modules.
- R19. In the first slice, the core-owned settings boundary is limited to the shared non-interactive contract needed by multiple surfaces, including user-config path resolution, parse/save behavior, and effective runtime assembly; interactive prompts, setup-step orchestration, malformed-config recovery UX, and other terminal-specific setup policy remain CLI-owned.
- R20. The first-slice analysis facade exposes one reusable execution contract that future surfaces can call; terminal rendering remains outside that contract.
- R21. The first-slice analysis facade is async-first and returns analysis state or a similarly typed analysis result rather than terminal-formatted output.
- R22. Low-level implementation modules such as agent definitions, workflow tasks, provider factories, raw data clients, and rig tool wrappers are internal to the core crate by default and are not the primary shared API for future surfaces.
- R23. Terminal-specific presentation concerns such as banners, interactive prompts, update notices, and terminal report formatting are excluded from the core crate's stable shared API.
- R24. The core crate reduces broad re-export patterns only as needed to support the new cross-crate seam in the first slice; broader export cleanup is deferred.
...
- The CLI uses a core-owned application facade for analysis execution instead of rebuilding the full runtime assembly inside CLI-specific code.
- The stable core API is smaller and more intentional: terminal UX concerns and low-level workflow internals are not treated as the primary shared surface.
...
- No opportunistic namespace cleanup, broad public API reshaping, or broad module churn beyond the boundary split.
- No full domain/application/infrastructure rewrite in the first slice.
...
- Application-centered core: expose a small runtime and analysis facade rather than exporting the current module tree as the future stable API.
- Internal-by-default core: agents, graph tasks, provider wiring, market-data plumbing, and tool adapters stay behind the core facade unless a real second consumer proves they need direct access.
- Settings ownership move: persisted user-settings behavior and effective runtime assembly move to the core side so CLI becomes a consumer of shared settings behavior rather than the owner of it.
- Presentation separation: terminal report rendering and other terminal-specific UX stay outside the stable core API.
...
- [Affects R16, R17, R20, R21, R24][Technical] What is the thinnest first-slice core facade shape, and which currently public modules or re-exports must remain temporarily available for in-repo callers during migration?
- [Affects R18, R19][Technical] What is the minimal shared non-interactive settings seam that removes the CLI dependency from runtime code without redesigning config behavior or terminal-specific setup UX?
- [Affects R20, R21, R23][Technical] Which result data or typed contracts need to come from core for the existing CLI path, while keeping terminal rendering explicitly CLI-owned?
